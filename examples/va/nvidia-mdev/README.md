...
3. [Configure and deploy the initial dataplane](dataplane.md)
4. After deploying dataplane install the necessary Nvidia Drivers on the EDPM Computes and blacklist nouveau.
5. Create and apply provider.yaml to a ConfigMap that is in turn applied to a OSPDS that maps the provided MDev resources to CUSTOM_TRAITS.
6. Execute a new deployment that applies servicesOverride that reboot's the EDPM Computes and applies the provider.yaml that was created in the previous stage.
